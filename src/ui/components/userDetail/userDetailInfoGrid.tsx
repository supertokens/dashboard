...
const NameTooltip: FC<{ fieldName: string }> = ({ fieldName }) => (
  <>
    <p className='center'>
      To change this information, please add / change <span className='block-snippet-small'>{fieldName}</span> key in
      the user metadata section below.
    </p>
    <p className='center'>
...
export const UserDetailInfoGridItem: FC<UserDetailInfoGridItemProps> = ({ label, body, tooltip }) => {
    ...
    tooltip !== undefined ? (
      <TooltipContainer tooltip={tooltip}>
        <span className='user-detail__info-grid__item__guide'>
          <img src={getImageUrl('help-icon.png')} alt={`${label} guideline`} />
        </span>
      </TooltipContainer>
    ) : null
  ...
  return (
    <div className='user-detail__info-grid__item'>
      ...
      <div className='user-detail__info-grid__item__body' title={typeof body === 'string' ? body : undefined}>
        {body ?? '-'}
      </div>
    </div>
  )
  ...
  <>
    ...
    {isEditing ? (
      <div className='actions'>
        <button className='button outline small' onClick={onCancel}>
          Cancel
        </button>
        <button className='button link outline small' onClick={onSave} disabled={isSaveDisabled}>
          Save
        </button>
      </div>
    ) : (
      <button className='button flat link small' onClick={onEdit}>
        Edit Info
      </button>
    )}
  </>
...
export const EmailVerifiedField: FC<EmailVerifiedFieldProps> = (props: EmailVerifiedFieldProps) => {
  ...
  return (
    <>
      {isVerified ? 'Yes' : 'No'}
      {isEditing && (
        <button className='flat link email-verified-button' onClick={() => setVerificationStatus(!isVerified)}>
          Set as {isVerified ? 'unverified' : 'verified'}
        </button>
      )}
      {!isEditing && !isVerified && (
        <button className='flat link email-verified-button' onClick={sendVerification}>
      ...
      <LayoutPanel
        ...
        <div className='user-detail__info-grid__grid'>
          ...
          <UserDetailInfoGridItem
            ...
            body={
              recipeId === 'emailpassword' ? (
                <button className='flat link' onClick={openChangePasswordModal}>
                  Change Password
                </button>
              ) : (
                NON_APPLICABLE_TEXT
          ...
          <UserDetailInfoGridItem
            ...
            body={recipeId === 'thirdparty' ? <UserDetailProviderBox user={userState.user} /> : NON_APPLICABLE_TEXT}
          />
        </div>
      </LayoutPanel>
    </div>
