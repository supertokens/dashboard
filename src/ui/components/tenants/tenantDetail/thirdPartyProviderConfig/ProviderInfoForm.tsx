				...
				{(emailSelectValue === "sometimes" || emailSelectValue === "never") && (
					<div className="overridden-info">
						...
						{emailSelectValue === "never"
							...
							: "Add a custom override for the getUserInfo method for this provider to handle the case when provider doesn't return email. You can also choose to generate a fake email using the option below, when the provider doesn't return email (to override how the fake email is generated you can override generateFakeEmail in the provider config)."}
					</div>
				)}

				{emailSelectValue === "sometimes" && (
					<div className="fields-container__toggle-container">
						<ThirdPartyProviderInputLabel label="Do you want to generate a fake email when the provider doesn't return email?" />
						<Toggle
							id="requireEmail"
