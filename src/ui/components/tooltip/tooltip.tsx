...
export const TooltipPopup: FC<TooltipPopupProps> = ({ duration, children, onDisappear, properties, tooltipWidth }) => {
  ...
  return <TemporaryContent duration={ duration } onDisappear={onDisappear}>
    <div className={`tooltip-container__popup popup_${properties?.positionType}`} data-theme="dark" style={{ maxWidth: `${tooltipWidth}px`, width, ...properties?.css }}>{children}</div>
  </TemporaryContent>
}
